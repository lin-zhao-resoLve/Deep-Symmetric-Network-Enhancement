...
def get_len(route, phase):
    if phase =='train':
        train_low_data_names = glob(route + 'low/*.jpg')
        train_low_data_names.sort()
        train_high_data_names = glob(route + 'high/*.jpg')
        train_high_data_names.sort()
        return len(train_high_data_names),train_low_data_names,train_high_data_names
    elif phase =='test':
        test_low_data_names = glob(route + 'low/*.jpg')
        test_low_data_names.sort()
        test_high_data_names = glob(route + 'high/*.jpg')
        test_high_data_names.sort()
        return len(test_low_data_names), test_low_data_names,test_high_data_names
    elif phase == 'eval':
        eval_low_data_names = glob(route + 'low/*.jpg')
        eval_low_data_names.sort()
        eval_high_data_names = glob(route + 'high/*.jpg')
...
class MTCDataset(Dataset):
    def __init__(self, opt):
        ...
        self.input_images = [None]*5000
        self.gt_images = [None]*5000
        self.num = [0]*5000
        self.pre = [0]*5000
    ...
    def __getitem__(self,index):
        if self.phase == 'train':
            ...
            # w = low_im.shape[1]
            # if h % 2 != 0 and h != 500:
            #     low_im = np.concatenate([low_im, np.expand_dims(low_im[0], 0)], 0)
            #     high_im = np.concatenate([high_im, np.expand_dims(high_im[0], 0)], 0)
            # elif w % 2 != 0 and w != 500:
            #     low_im = np.concatenate([low_im, np.expand_dims(low_im[:, 0, :], 1)], 1)
            #     high_im = np.concatenate([high_im, np.expand_dims(high_im[:, 0, :], 1)], 1)
            ...
            if h % 2 != 0 and h != 500:
                low_im = np.concatenate([low_im, np.expand_dims(low_im[0], 0)], 0)
                high_im = np.concatenate([high_im, np.expand_dims(low_im[0], 0)], 0)
            elif w % 2 != 0 and w != 500:
                low_im = np.concatenate([low_im, np.expand_dims(low_im[:, 0, :], 1)], 1)
                high_im = np.concatenate([high_im, np.expand_dims(low_im[:, 0, :], 1)], 1)
            return {'LQ': TF.ToTensor()(low_im), 'GT': TF.ToTensor()(high_im), 'raw_path': self.low_names[index], 'image_shape':[h, w]}
    def __len__(self):
        return self.len
