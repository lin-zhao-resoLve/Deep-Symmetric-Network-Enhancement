...
file1 = '/home/lin-zhao/DEEPLPF/no_1'
file2 = '/home/lin-zhao/C_4500_500/test/high'
filename1, name1s = store_dataset(file1)
filename2, name2s = store_dataset(file2)
psnr1s = []
# psnr2s = []
# psnr3s = []
...
for i in range(len(filename1)):
    print(name1s[i])
    print(name2s[i])
    img1 = cv2.imread(filename1[i], cv2.COLOR_BGR2RGB)
    img2 = cv2.imread(filename2[i], cv2.COLOR_BGR2RGB)
    psnr1 =calc_psnr(img1, img2)
    psnr1s.append(psnr1)
    ssim1 = calc_ssim(img1,img2)
    ssim1s.append(ssim1)
    print('i:%d,ours:%f,%f; '%(i, psnr1, ssim1, ))

print('ours:%f,%f; '%(sum(psnr1s)/len(psnr1s), sum(ssim1s)/len(ssim1s),))
